...
class PacmanEnv:
    def __init__(self):
        ...
        self.reset() # Initialisation complète (Niveau 1)

    def reset(self):
        """Reset complet (Nouvelle partie / Épisode d'apprentissage)"""
        self.level = 1
        self.score = 0
        self.lives = INITIAL_LIVES
        self.ghosts_eaten = 0 
        self.last_action = 4 # Action neutre au départ
        return self._start_level()

    def next_level(self):
        """Passe au niveau suivant (garde score et vies)"""
        self.level += 1
        return self._start_level()

    def _start_level(self):
        """Prépare la grille pour le niveau actuel"""
    ...
    def _reset_positions(self):
        # Trouvons une case vide sure.
        # (1, 6) est un mur.
        # (5, 5) est un 2 (point).
        self.pacman_pos = (5, 5) # Position sûre
    ...
    def step(self, action):
        ...
        if cell == DOT:
            ...
            self.total_dots -= 1
            self.score += 50
            for g in self.ghosts: g['scared'] = self.power_duration # Durée dynamique

        # Victoire (Fin de niveau)
        if self.total_dots == 0:
            if self.level >= MAX_LEVEL:
                ...
                self.done = True # Fin de séquence (mais pas forcément fin de partie pour main.py)
                info["level_cleared"] = True
                return self.get_state(), R_WIN, True, info

        # Fantômes (Mouvement probabiliste selon difficulté)
        for g in self.ghosts:
            if g['scared'] > 0: g['scared'] -= 1
            
            # Test de mouvement (vitesse simulée)
    ...
    def get_state(self):
        """
        État Radar Optimisé pour la Chasse.
        """
        ...
        for dr, dc in deltas:
            ...
            # 2. Statut Fantôme
            ghost_status = 0 
            # On regarde un peu plus loin pour voir les opportunités
            for i in range(1, 5):
                r, c = head_r + (dr*i), head_c + (dc*i)
                for g in self.ghosts:
                    if g['pos'] == (r, c):
                        if g['scared'] > 0: 
                            ghost_status = 2 
            ...
            if target:
                ...
                dist_next = abs(target[0] - (head_r+dr)) + abs(target[1] - (head_c+dc))
                if dist_next < dist_current:
                    food_dir = 1

            radar.append((is_wall, ghost_status, food_dir))
            
        any_scared = 1 if any(g['scared'] > 0 for g in self.ghosts) else 0
        
        return tuple(radar + [any_scared, self.last_action])
